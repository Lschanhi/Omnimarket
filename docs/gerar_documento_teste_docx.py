...
PLANO_SOURCE = ROOT / "PLANO_DE_TESTE.md"
MANUAL_SOURCE = ROOT / "MANUAL_DE_TESTE.md"
OUTPUT = ROOT / "Documento_Teste_Cadastro_Email_OmniMarket.docx"
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties"
  ...
  <dc:title>Documento de Teste Cadastro Email OmniMarket</dc:title>
  <dc:subject>Plano e Manual de Teste</dc:subject>
...
def read_markdown_sections() -> list[str]:
    plano_lines = PLANO_SOURCE.read_text(encoding="utf-8").splitlines()
    manual_lines = MANUAL_SOURCE.read_text(encoding="utf-8").splitlines()
    data_geracao = datetime.now().strftime("%d/%m/%Y %H:%M")

    lines: list[str] = [
        "# Documento de Teste - Cadastro com Confirmacao de Email",
        "",
    ...
    lines.extend(plano_lines)
    lines.extend(
        [
            "",
            "{{PAGE_BREAK}}",
            "",
            "## Parte 2 - Manual de Teste",
            "",
        ]
    )
